return async function (rowData) {
  ...
  try {
    ...
    const processOperationsAndMaterials = async (workOrder, part, rowData) => {
    ...
    //  let methodOperations = $getGrid("methodOperations").filter((op) => op.parentPart === part);
      ...
      let methodOperations = $getGrid('methodOperations').filter(row => row.productionMethod === rowData.productionMethod);
      ...
      let nops = $getGrid('workOrderOperations').filter((nop) => nop.workOrder === rowData.rowKey);
...
*/
    ...
    await processOperationsAndMaterials(currentWorkOrder, parentPart, rowData);
