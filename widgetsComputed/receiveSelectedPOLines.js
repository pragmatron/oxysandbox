return async function (data) {
  ...
  if (selectedPurchaseLines.length < 1) {
    ...
    let filteredSelectedLines = $getGrid('purchaseLines')
      .filter((line) => selectedPurchaseLines.includes(line.rowKey))
      .filter((line) => line.remainingQuantity > 0)

    if (filteredSelectedLines.length === 0) {
      alert('No lines available to receive')
      return
    }
    ...
    for (const [index, line] of filteredSelectedLines.entries()) {
      ...
      try {
        ...
        const qty = line.quantityToReceive
        ...
        let newlot = await $dgAddRow('locationInventory', {
        //  name: `${nextId}.0${index+1}`,
          quantity: qty,
          available: qty,
